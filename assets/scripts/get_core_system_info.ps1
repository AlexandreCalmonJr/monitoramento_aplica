...
try {
    ...
    # ✅ COLETA DO BSSID (MAC ADDRESS RADIO)
    ...
    $macAddressRadio = if ($wifiAdapter) { 
        $wifiAdapter.MacAddress 
    } else { 
        "N/A" 
    }
    ...
    $systemInfo = @{
        ...
        mac_address = $wifiAdapter.MacAddress  # MAC do adaptador
        mac_address_radio = $macAddressRadio   # ✅ BSSID (crucial!)
        connection_type = if ($wifiAdapter) { "WiFi" } else { "Ethernet" }
    }
    
    # ✅ SE FOR WIFI, ADICIONA INFORMAÇÕES EXTRAS
    if ($wifiAdapter) {
        try {
            $wifiProfile = netsh wlan show interfaces | Select-String "SSID", "BSSID", "Signal"
            $ssidLine = $wifiProfile | Where-Object { $_ -match "SSID\s+:\s+(.+)" }
            $bssidLine = $wifiProfile | Where-Object { $_ -match "BSSID\s+:\s+(.+)" }
            $signalLine = $wifiProfile | Where-Object { $_ -match "Signal\s+:\s+(\d+)%" }
            
            if ($ssidLine) {
                $systemInfo.wifi_ssid = $Matches[1].Trim()
            }
            if ($bssidLine) {
                $systemInfo.mac_address_radio = $Matches[1].Trim()  # ✅ Sobrescreve com BSSID real
            }
            if ($signalLine) {
                $systemInfo.wifi_signal = "$($Matches[1])%"
            }
        } catch {
            # Ignora erros de WiFi
        }
    }
    
    # Retorna JSON
    $systemInfo | ConvertTo-Json -Compress
